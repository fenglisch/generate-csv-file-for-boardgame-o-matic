...
const arFilterOptionsThemes = [
   ...
   },
   {
      value: "trains",
      selector:
         "[value='Racing'], [value='Trains'], [value='Aviation / Flight']",
   },
   {
      value: "war",
      selector:
   ...
   try {
      const formInput = (function readUserInputFromForm() {
         const input = {
            ...
            isWantedInventoryLocation: document.querySelector(
               "#inventory-location"
            ).checked,
         };
    ...
    &stats=1`
            ...
            document
               ...
               .addEventListener("click", () => {
                  ...
                  const parserError =
                     xmlCollection.querySelector("parsererror");
                  if (parserError) {
                     console.error(
         ...
         nlAllGamesInCollection.forEach((nodeGame) => {
            const game = {
               ...
               arLanguages: nodeGame.querySelector("version")
                  ? Array.from(
                       nodeGame.querySelectorAll("[type='language']")
                    ).map((nodeLanguage) => nodeLanguage.getAttribute("value"))
                  : null,
               thumbnail:
   ...
   const arGamesDetailedData = await (async function getDetailedDataForGames(
      ...
      for (let i = 0; i < arGameIdClusters.length; i++) {
         ...
         for (let j = 0; j < nlAllGamesOfCluster.length; j++) {
            ...
            game.questionValues =
               (function getValuesForBoardGameOMaticQuestions(xmlGame, game) {
                  ...
                  function getPlayTimeGroup(xmlGame) {
                     const playTime =
                        +xmlGame
                           ...
                           .getAttribute("value") * 1.25 ?? null; // Official max time is multiplied by 1.25 to get more realistic play time
                     if (!playTime)
                  ...
                  function getLanguageDependenceGroup(xmlGame) {
                     const poll = xmlGame.querySelector(
            ...
            game.filterValues = (function getValuesForBoardGameOMaticFilters(
               ...
               filterValues.playerNumbers =
                  (function getValuesForFilterPlayerNumber(
                     xmlGame,
                     maxPlayerCount
                  ) {
                     ...
                     for (let i = 1; i <= maxPlayerCount; i++) {
                        if (isRecommended(i, xmlGame))
                           arRecommendedPlayerCount.push(i);
                     }
               ...
               filterValues.mechanics = (function getValuesForFilterMechanics(
                  ...
                  if (
                     !arMechanics.includes("areaControl") &&
               ...
               filterValues.themes = (function getValuesForFilterThemes(
                  xmlGame
               ) {
                  const arThemes = [];
            ...
            game.descriptions = await (async function getDescriptions(
               ...
               if (!isWantedChatGpt) {
                  descriptions.bgg =
                     await (async function getDescriptionsLongBgg(
                        descriptions,
                        xmlGame,
                        isWantedTranslate
                     ) {
...
"Beschreibung";"${
            game.descriptions.chatGpt[language] ||
            game.descriptions.bgg[language]
         } ${
